...
segmentation_id = 0
numclasses = 3 # excluding background
images = sorted(glob.glob(os.path.join(IMAGE_DIR, "*.png"), recursive=True))[:10]
gts = sorted(glob.glob(os.path.join(LABEL_DIR, "*.png"), recursive=True))[:10]
print(len(images))
print(len(gts))
for image_id, image_filename in enumerate(images):
    ...
    # else:
    #     class_id = 3
    for class_id in range(numclasses):
        ...
        for u in np.unique(instance_map)[1:]: # exclude background
            ...
            subinstance_mask = subinstance_mask
            # Image.fromarray(subinstance_mask).save(os.path.join(BIN_SEGDIR, f"mask_{image_id}_{class_id}_{segmentation_id}.png"))
            
            # annotation_info = pycococreatortools.create_annotation_info(
            #     segmentation_id, image_id, category_info, subinstance_mask,
            #     image.size, tolerance=2)
            annotation_info = ann_from_mask_rle(segmentation_id, image_id, class_id, subinstance_mask, iscrowd=0)

...
class CocoSegmentationDataset(Dataset):
    ...
    def __init__(
        ...
        root_dir: str,
        annotation_file: str,
        transforms: Callable = get_transform(),
    ...
    def __getitem__(self, idx: int):
        ...
        if masks:
            ...
            # print("np.sum(masks_array):", np.sum(masks_array))
            if np.sum(masks_array) == 0:
                print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                print(
                    f"CRITICAL WARNING: Image {image_id} has annotations, but ALL ground-truth masks are EMPTY."
                )
...
#             np.array([ann["area"] for ann in annotations]), dtype=torch.float32
#         )
#         iscrowd = torch.as_tensor(
#             np.array([ann.get("iscrowd", 0) for ann in annotations]), dtype=torch.int64
#         )
...
def build_model(num_classes: int) -> torch.nn.Module:
    ...
    model.roi_heads.box_predictor = FastRCNNPredictor(
        in_channels=in_features_box, num_classes=num_classes
    )
...
def fit(
    ...
    for epoch in range(start_epoch + 1, epochs + 1):
        ...
        if tracker is not None:
            ...
            all_metrics = {"epoch": epoch, "val_loss": val_loss}
        ...
        checkpoint_path = out / f"epoch_{epoch:03d}.pth"
        torch.save(
            {
                "epoch": epoch,

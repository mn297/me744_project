...
def compute_instance_metrics(pred_masks, target_masks, iou_threshold=0.5):
    """
    Compute mean IoU and mean Dice for matched instances.
    Simple iterative implementation.

    Args:
        pred_masks: boolean array [N, H, W]
        target_masks: boolean array [M, H, W]
        iou_threshold: threshold for considering a match valid

    Returns:
        mean_iou, mean_dice, num_matches
    """
    ...
    # Ensure boolean
    preds = pred_masks > 0
    targs = target_masks > 0

    # Calculate all pairwise metrics
    # List of (iou, dice, pred_idx, gt_idx)
    matches = []
    ...
    for i, p in enumerate(preds):
        for j, t in enumerate(targs):
            ...
            if iou >= iou_threshold:
                matches.append((iou, dice, i, j))

    # Sort by IoU descending to greedily pick best matches
    ...
    for iou, dice, p_idx, g_idx in matches:
    ...
    return np.mean(final_ious), np.mean(final_dices), len(final_ious)


def compute_global_metrics(pred_masks, target_masks):
    """
    ...
    # Collapse all masks into one binary mask per set (Union)
    # Any pixel that is part of ANY object is True
...
def draw_prediction(
    ...
    if target:
        ...
        if len(masks) > 0 and len(target["masks"]) > 0:
            ...
            mean_iou, mean_dice, n_match = compute_instance_metrics(pm, tm)
            global_iou, global_dice = compute_global_metrics(pm, tm)

            stats_text += f"\nMatched (IoU>0.5): {n_match}"
            stats_text += f"\nInstance Mean IoU: {mean_iou:.3f}"
            stats_text += f"\nInstance Mean Dice: {mean_dice:.3f}"
            stats_text += f"\nGlobal IoU: {global_iou:.3f}"
            stats_text += f"\nGlobal Dice: {global_dice:.3f}"
...
def visualize_coco_dataset(
    ...
    for idx in tqdm(indices):
        ...
        # Save
        # We want to save with the same stem as the original filename if possible.
        # The dataset object has 'image_ids'. We need to map image_id back to filename.
        # CocoSegmentationDataset does this internally in _load_image, let's access it.

        # Try to get filename from dataset
        try:
